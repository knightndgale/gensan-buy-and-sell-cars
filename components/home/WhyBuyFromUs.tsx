import { DollarSign, Fuel, Heart, MessageSquare } from "lucide-react";
...
export function WhyBuyFromUs() {
  return (
    <div className="container mx-auto max-w-7xl px-3 sm:px-4 ">
      ...
      <p className="mb-10 text-xs sm:text-base text-muted-foreground">Our platform is designed for families, small business owners, and everyday drivers who value:</p>

      <div className="grid gap-6 sm:grid-cols-3">
        {BENEFITS.map(({ icon: Icon, description }) => (
          <div key={description} className="flex flex-row items-center sm:items-start gap-4 rounded-xl border border-border bg-muted p-6">
            <div className="flex size-12 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary">
              <Icon className="size-5 sm:size-6" />
            </div>
            <p className="text-xs sm:text-base text-foreground">{description}</p>
          </div>
        ))}
      </div>
