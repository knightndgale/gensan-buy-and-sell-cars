...
export default async function CarDetailPage({ params }: PageProps) {
  ...
  const detailsContent = (
    <section className="h-fit px-0 pt-4 lg:pt-0" aria-labelledby="car-detail-title">
      <h1 id="car-detail-title" className="text-2xl font-bold text-foreground">
        {title}
      </h1>
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="flex flex-col items-center gap-1 text-center">
          ...
          <span className="text-sm font-medium text-foreground">{mileageLabel}</span>
        </div>
        <div className="flex flex-col items-center gap-1 text-center">
          <Fuel className="size-5 text-primary" />
          <span className="text-xs text-muted-foreground">Fuel</span>
          <span className="text-sm font-medium text-foreground">{listing.fuelType ? formatFuelType(listing.fuelType) : "—"}</span>
        </div>
        <div className="flex flex-col items-center gap-1 text-center">
          <Settings className="size-5 text-primary" />
          <span className="text-xs text-muted-foreground">Transmission</span>
          <span className="text-sm font-medium text-foreground">{listing.transmission ? formatTransmission(listing.transmission) : "—"}</span>
        </div>
        <div className="flex flex-col items-center gap-1 text-center">
          <Calendar className="size-5 text-primary" />
          <span className="text-xs text-muted-foreground">Year</span>
          <span className="text-sm font-medium text-foreground">{yearLabel}</span>
        </div>
      </div>

      <Separator className="my-4" />

      <section aria-labelledby="car-description-heading">
        <h2 id="car-description-heading" className="text-base font-semibold text-foreground">
          Description
        </h2>
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground whitespace-pre-line">{listing.description?.trim() || "—"}</p>
      </section>

      <Separator className="my-4" />

      <section aria-labelledby="car-specs-heading">
        <h2 id="car-specs-heading" className="text-base font-semibold text-foreground">
          Specifications
        </h2>
        <div className="mt-3 grid grid-cols-2 gap-3">
          <div className="flex items-center gap-2 rounded-lg bg-muted px-3 py-2">
            ...
            <div className="min-w-0">
              ...
              <span className="text-sm font-medium text-foreground">{listing.bodyType || "—"}</span>
            </div>
          </div>
          <div className="flex items-center gap-2 rounded-lg bg-muted px-3 py-2">
            ...
            <div className="min-w-0">
              ...
              <span className="text-sm font-medium text-foreground">{listing.engine || "—"}</span>
            </div>
          </div>
          <div className="flex items-center gap-2 rounded-lg bg-muted px-3 py-2">
            ...
            <div className="min-w-0">
              ...
              <span className="text-sm font-medium text-foreground">{listing.color || "—"}</span>
            </div>
          </div>
          <div className="flex items-center gap-2 rounded-lg bg-muted px-3 py-2">
      ...
      {resolvedFeatures.length > 0 && (
        <>
          <Separator className="my-4" />
          <section aria-labelledby="car-features-heading">
            <h2 id="car-features-heading" className="text-base font-semibold text-foreground">
              Features
            </h2>
            <div className="mt-3 flex flex-wrap gap-2">
              {resolvedFeatures.map((f) => (
                <span key={f.id} className="rounded-full bg-primary/10 px-3 py-1.5 text-sm font-medium text-primary">
                  {f.name}
                </span>
              ))}
            </div>
      ...
      {dealer && (
        <>
          <Separator className="my-4" />
          <section aria-labelledby="car-seller-heading">
            <h2 id="car-seller-heading" className="text-base font-semibold text-foreground">
              Seller
            </h2>
            <div className="mt-3 flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="font-medium text-foreground">{dealer.dealershipName}</p>
      ...
      <section id="car-detail-contact-section">
        <CarDetailContactSection dealer={dealer} listingId={id} carName={title} isAdmin={isAdmin} listingStatus={listing.status as "active" | "pending" | "sold"} />
      </section>
    </section>
  );
